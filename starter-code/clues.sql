...
SELECT country, population, region  FROM country WHERE region LIKE '%Southern Europe%' ORDER BY population;
	Vatican City State
...
SELECT countrycode, language, percentage FROM countrylanguage  WHERE percentage = '100'; SELECT name FROM country WHERE code = 'SMR';
	San Marino
...
SELECT country.capital FROM country WHERE code = 'BRA'; SELECT name FROM city WHERE id = 211;
	Brasïlia
